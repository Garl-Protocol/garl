-- v18 — Wave 2 foundation: receipts, capability_tokens, compensations, merkle_batches
--
-- Generic receipts table (Action Receipt v0.1 envelope) is additive next to
-- the existing `traces` table. Both stay valid; new submissions go to
-- `receipts`, legacy /verify writes still go to `traces`. The two tables
-- merge into one in v25+ once consumers have migrated.

-- ──────────────────────────────────────────────────────────────────────
-- 1) receipts — Action Receipt v0.1 generic envelope
-- ──────────────────────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_receipts_pending_anchor ON receipts(created_at)
  WHERE anchored_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_receipts_output_hash ON receipts(output_hash);

-- Immutability: only `anchored_at` and `merkle_batch_id` may be set after
-- insert (by the batch builder). Identity/signature/envelope are frozen.
...
CREATE TRIGGER receipts_immutable_update
  ...
  FOR EACH ROW EXECUTE FUNCTION prevent_receipt_update();
...
CREATE POLICY receipts_public_read ON receipts
  FOR SELECT TO public
  USING (
    redaction_policy IS NULL
    OR (redaction_policy->>'mode' IS DISTINCT FROM 'private')
  );
...
-- ──────────────────────────────────────────────────────────────────────
-- 2) capability_tokens — JWT-shaped + ECDSA-secp256k1
-- ──────────────────────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_capability_tokens_active ON capability_tokens(agent_id, expires_at)
  WHERE revoked_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_capability_tokens_parent ON capability_tokens(parent_token_hash)
...
CREATE POLICY capability_tokens_service_role_all ON capability_tokens
  FOR ALL TO service_role USING (true) WITH CHECK (true);

COMMENT ON TABLE capability_tokens IS 'Issued capability tokens. JWT-shaped envelope (header.payload.signature) signed with ECDSA-secp256k1 RFC 6979 deterministic. Attenuation: parent_token_hash chains.';

-- ──────────────────────────────────────────────────────────────────────
-- 3) compensations — UETA §10(b) consumer-undo + general reversibility
-- ──────────────────────────────────────────────────────────────────────
...
CREATE POLICY compensations_service_role_all ON compensations
  FOR ALL TO service_role USING (true) WITH CHECK (true);

COMMENT ON TABLE compensations IS 'Reversibility primitives. status flow: recorded (at receipt time) -> pending (consumer triggered undo) -> succeeded|failed (after the undo action ran).';

-- ──────────────────────────────────────────────────────────────────────
-- 4) merkle_batches — periodic on-chain anchoring
-- ──────────────────────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_merkle_batches_pending ON merkle_batches(built_at)
  WHERE anchored_at IS NULL;

ALTER TABLE merkle_batches ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS merkle_batches_public_read ON merkle_batches;
CREATE POLICY merkle_batches_public_read ON merkle_batches
  FOR SELECT TO public USING (true);

DROP POLICY IF EXISTS merkle_batches_service_role_all ON merkle_batches;
CREATE POLICY merkle_batches_service_role_all ON merkle_batches
  FOR ALL TO service_role USING (true) WITH CHECK (true);

COMMENT ON TABLE merkle_batches IS 'Periodic Merkle root commitments. anchored_at NULL = built but not yet on-chain.';

-- Backref from receipts to merkle_batches once a batch is built
ALTER TABLE receipts
  DROP CONSTRAINT IF EXISTS receipts_merkle_batch_fk;
ALTER TABLE receipts
  ADD CONSTRAINT receipts_merkle_batch_fk
  FOREIGN KEY (merkle_batch_id) REFERENCES merkle_batches(batch_id);
